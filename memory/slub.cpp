...
void Slub::cmd_main(void) {
    ...
    if (cache_list.size() == 0){
        LOGD("Initializing cache database...");
        parser_slab_caches();
    } else {
        LOGD("Using cached database (%zu caches)", cache_list.size());
    }
    while ((c = getopt(argcnt, args, "asc:pP:t:lT:L:v:")) != EOF) {
        ...
        switch(c) {
            case 's':
                LOGD("Command: Display memory summary");
                print_slab_summary_info();
                break;
            case 'a':
                LOGD("Command: Display all caches");
                print_slab_caches();
                break;
            case 'c':
                LOGD("Command: Display cache %s", optarg_str.c_str());
                print_slab_cache_info(optarg_str);
                break;
            case 'p':
                LOGD("Command: Check all caches for corruption");
                print_slub_poison();
                break;
            case 'P':
                LOGD("Command: Check cache %s for corruption", optarg_str.c_str());
                func_call(optarg_str, "kmem_cache_addr", [this](const std::string& s) {
                    print_slub_poison(std::stoul(s, nullptr, 16));
                });
                break;
            case 't':
                LOGD("Command: Display %s traces", optarg_str.c_str());
                print_slub_trace(optarg_str);
                break;
            case 'l':
                LOGD("Command: Display all traces");
                print_all_slub_trace();
                break;
            case 'L':
                LOGD("Command: Display traces for cache %s", optarg_str.c_str());
                print_cache_specific_trace(optarg_str);
                break;
            case 'T':
                LOGD("Command: Analyze stack_id %s", optarg_str.c_str());
                func_call(optarg_str, "stack_id", [this](const std::string& s) {
                    print_all_slub_trace(std::stoul(s));
                });
                break;
            case 'v':
                LOGD("Command: Find object at address %s", optarg_str.c_str());
...
std::shared_ptr<slab> Slub::parser_slab(std::shared_ptr<kmem_cache> cache_ptr, ulong slab_page_addr){
    ...
    if(page_buf == nullptr) {
        LOGE("Failed to read slab/page structure at %#lx for cache %s", slab_page_addr, cache_ptr->name.c_str());
        return nullptr;
    }
    ...
    while (is_kvaddr(fobj)){
        ...
        if (obj_index < 0 || obj_index >= slab_ptr->totalobj || obj_free[obj_index] == 1){
            ...
            LOGE("Corrupted freelist detected in cache %s: obj_index=%d, totalobj=%u (fobj=%#lx, slab_vaddr=%#lx)",
                 cache_ptr->name.c_str(), obj_index, slab_ptr->totalobj, fobj, slab_vaddr);
            break;
        }
...
std::vector<std::shared_ptr<slab>> Slub::parser_slab_from_list(std::shared_ptr<kmem_cache> cache_ptr, ulong head_addr){
    ...
    for (const auto& slab_page_addr : page_list) {
        if (std::find(temp_list.begin(), temp_list.end(), slab_page_addr) != temp_list.end()) {
            continue;
        }
    ...
    LOGD("  Found %zu slab(s) in list", slab_list.size());
    return slab_list;
}
...
std::vector<std::shared_ptr<kmem_cache_node>> Slub::parser_kmem_cache_node(std::shared_ptr<kmem_cache> cache_ptr, ulong node_addr){
    ...
    for (int i = 0; i < node_cnt; i++){
        ...
        if(node_buf == nullptr) {
            LOGE("  Node %d: failed to read structure at %#lx", i, addr);
            continue;
        }
        ...
        LOGD("  Node[%d] partial: %zu slab(s), %lu objects", i, node_ptr->partial.size(), node_ptr->nr_partial);
        ulong full_addr = addr + field_offset(kmem_cache_node,full);
        node_ptr->full = parser_slab_from_list(cache_ptr,full_addr);
        LOGD("  Node[%d] full: %zu slab(s)", i, node_ptr->full.size());
        node_list.push_back(node_ptr);
    }
    if (node_list.size() > 0) {
        LOGD(" Parsed %zu NUMA node(s)", node_list.size());
    }
    return node_list;
}
...
std::vector<std::shared_ptr<kmem_cache_cpu>> Slub::parser_kmem_cache_cpu(std::shared_ptr<kmem_cache> cache_ptr, ulong cpu_addr){
    ...
    if ((kt->flags & SMP) && (kt->flags & PER_CPU_OFF)) {
        for (size_t i = 0; i < NR_CPUS; i++) {
            ...
            if(cpu_buf == nullptr) {
                LOGE("  CPU %zu: failed to read structure at %#lx", i, addr);
                continue;
            }
    ...
    if (cpu_list.size() > 0) {
        LOGD(" Parsed %zu per-CPU slab(s)", cpu_list.size());
    }
    return cpu_list;
}

void Slub::parser_slab_caches(){
    LOGI("Starting parser slub cache");
    cache_list.clear();
    ...
    LOGD("Scanning slab_caches list at %#lx", slab_caches_addr);
    ...
    for (const auto& cache_addr : for_each_list(slab_caches_addr,offset)) {
        ...
        if(cache_buf == nullptr) {
            LOGE("Failed to read kmem_cache structure at %#lx, skipping", cache_addr);
            continue;
        }
        cache_count++;
        ...
        LOGD("Cache #%zu: '%s' [%#lx] obj_size=%u flags=%u size=%u inuse=%u",
             cache_count, cache_ptr->name.c_str(), cache_addr, cache_ptr->object_size, cache_ptr->flags, cache_ptr->size, cache_ptr->inuse);
    ...
    max_name_len += 5;
    LOGI("Parsing complete: %zu caches, %zu slabs, %zu objects",
         cache_list.size(), total_slabs, total_objs);
    if (!has_store_user) {
        PRINT("WARNING: No SLUB cache has SLAB_STORE_USER flag enabled!\n");
        PRINT("         Stack trace features (-t, -l, -L, -T, -v) may not work properly.\n");
        PRINT("         To enable: Add 'slub_debug=U' to kernel boot parameters.\n");
    }
}
...
void Slub::parser_slub_trace() {
    if (trace_parsed) {
        LOGD("Using cached trace data");
        return;
    }
    LOGI("Building allocation/free trace database...");
    alloc_trace_map.clear();
    free_trace_map.clear();
    std::ostringstream oss;
    size_t caches_with_trace = 0;
    for (const auto& cache_ptr : cache_list) {
    ...
    trace_parsed = true;
    LOGI("Trace database built: %zu caches, %zu alloc stacks, %zu free stacks",
         caches_with_trace, alloc_trace_map.size(), free_trace_map.size());
}
...
void Slub::print_trace_results(const std::unordered_map<std::string, std::vector<std::shared_ptr<track>>>& trace_map,
    ...
    LOGI("Displaying %zu unique %s stack trace(s)", trace_map.size(), is_free ? "free" : "allocation");
    if (trace_map.empty()) {
        PRINT("No %s traces to display\n", is_free ? "free" : "alloc");
        return;
    }
    // sort by times
    ...
    LOGD("Stack trace display complete");
}

void Slub::print_slub_trace(std::string trace_type) {
    LOGI("Generating %s trace report...", trace_type == "A" || trace_type == "a" ? "allocation" : "free");
    parser_slub_trace();
    if (trace_type == "A" || trace_type == "a") {
        LOGD("Processing %zu allocation traces", alloc_trace_map.size());
        print_trace_results(alloc_trace_map, false);
    } else if (trace_type == "F" || trace_type == "f") {
        LOGD("Processing %zu free traces", free_trace_map.size());
        print_trace_results(free_trace_map, true);
    } else {
        LOGE("Invalid trace type: %s (expected 'A' or 'F')", trace_type.c_str());
    }
    LOGD("Trace report complete");
}

void Slub::print_all_slub_trace(size_t stack_id) {
    LOGI("Generating comprehensive trace report%s", stack_id ? " for specific stack" : "");
    parser_slub_trace();
    if (stack_id == 0) {
        LOGD("Showing all allocation and free traces");
        print_trace_results(alloc_trace_map, false);
        print_trace_results(free_trace_map, true);
    } else {
        LOGD("Analyzing stack_id=%zu", stack_id);
...
void Slub::print_cache_specific_trace(std::string cache_identifier) {
    LOGI("Generating trace report for cache: %s", cache_identifier.c_str());
    auto target_cache = find_cache_by_identifier(cache_identifier);
    if (!target_cache) {
        LOGE("Cache not found: %s", cache_identifier.c_str());
        return;
    }
    LOGD("Located cache '%s' at %#lx", target_cache->name.c_str(), target_cache->addr);
    if ((target_cache->flags & SLAB_STORE_USER) == 0) {
        LOGE("Cache %s does not have SLAB_STORE_USER flag (flags=%#x)",
             target_cache->name.c_str(), target_cache->flags);
        return;
    }
    ...
    LOGD("Cache-specific trace report complete");
}

std::shared_ptr<kmem_cache> Slub::find_cache_by_identifier(const std::string& identifier) {
    LOGD(" Looking up cache: %s", identifier.c_str());
    // find cache by addr
    try {
        ulong cache_addr = std::stoul(identifier, nullptr, 16);
        LOGD("  Searching by address: %#lx", cache_addr);
        for (const auto& cache_ptr : cache_list) {
            if (cache_ptr->addr == cache_addr) {
                LOGD("  Found: '%s'", cache_ptr->name.c_str());
                return cache_ptr;
            }
        }
        LOGD("  No match for address %#lx", cache_addr);
    } catch (...) {
        // find cache by name
        LOGD("  Searching by name: %s", identifier.c_str());
        for (const auto& cache_ptr : cache_list) {
            if (cache_ptr->name == identifier) {
                LOGD("  Found: '%s' at %#lx", cache_ptr->name.c_str(), cache_ptr->addr);
                return cache_ptr;
            }
        }
        LOGD("  No match for name '%s'", identifier.c_str());
    }
    LOGD(" Cache lookup failed");
    return nullptr;
}
...
void Slub::print_stack_details(size_t stack_id) {
    ...
    if (result.first.empty()) {
        LOGE("Stack ID %zu not found in trace maps", stack_id);
        return;
    }
    /**
     * Key: PID (Process ID)
...
void Slub::parser_track(ulong track_addr, std::shared_ptr<track>& track_ptr){
    ...
    if (!track_buf) {
        LOGE("Failed to read track structure at %#lx", track_addr);
        return;
    }
...
void Slub::print_slab_caches(){
    LOGI("Displaying detailed information for %zu cache(s)", cache_list.size());
    for (const auto& cache_ptr : cache_list) {
        print_slab_cache(cache_ptr);
    }
    LOGD("Cache listing complete");
}

void Slub::print_slab_cache(std::shared_ptr<kmem_cache> cache_ptr){
    LOGD(" Formatting cache '%s' details", cache_ptr->name.c_str());
...
void Slub::print_slab_cache_info(std::string addr){
    LOGI("Looking up cache at address: %s", addr.c_str());
    unsigned long number = std::stoul(addr, nullptr, 16);
    if (number <= 0){
        LOGE("Invalid cache address: %s", addr.c_str());
        return;
    }
    LOGD(" Searching cache list for %#lx", number);
    bool found = false;
    for (const auto& cache_ptr : cache_list) {
        if (cache_ptr->addr != number) {
            continue;
        }
        LOGD(" Match found: '%s'", cache_ptr->name.c_str());
        print_slab_cache(cache_ptr);
        found = true;
        break;
    }
    if (!found) {
        LOGE("Cache not found at address %#lx", number);
    } else {
        LOGD(" Cache information displayed");
    }
}

void Slub::print_slab_summary_info(){
    LOGI("Generating memory usage summary for %zu cache(s)", cache_list.size());
    std::sort(cache_list.begin(), cache_list.end(),[&](const std::shared_ptr<kmem_cache>& a, const std::shared_ptr<kmem_cache>& b){
        return a->total_size > b->total_size;
    });
    LOGD(" Sorted by memory usage, formatting output");
    std::ostringstream oss;
...
void Slub::print_object_trace_by_addr(std::string addr_str) {
    ...
    LOGI(" Object found in cache '%s' - index:%d status:%s",
         found_cache->name.c_str(), found_obj->index, found_obj->is_free ? "freed" : "allocated");
