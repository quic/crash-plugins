...
void SF::dump_GraphicBufferAllocator(ulong sAllocList_vaddr){
    ...
    for (size_t i = 0; i < mCount; i++) {
        ...
        size_t size = 0;
        if(BITS64() && !task_ptr->is_compat()){
            size = task_ptr->uread_ulong(alloc_rec_t_addr + sizeof(alloc_rec_t));
        } else {
            size = task_ptr->uread_uint(alloc_rec_t_addr + sizeof(alloc_rec_t));
        }
...
void SF::init_command(){
    ...
    help_str_list={
        ...
        "-a \n"
            "  sfg -a\n"
            "  This command dump surfaceflinger gralloc4 info.",
        "-b \n"
            "  sfg -b\n"
            "  This command dump surfaceflinger gralloc5 info.",
        "\n",
        "EXAMPLES",
